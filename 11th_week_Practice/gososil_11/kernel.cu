...
__global__ void Mat_Vec_Multiply_Kernel(float *d_VecY, float *d_VecX, float *d_MatA, int Vec_Size)
{
	int row = blockDim.y*blockIdx.y + threadIdx.y;
	int col = blockDim.x*blockIdx.x + threadIdx.x;
	if (row < Vec_Size && col < Vec_Size) {
		float value = 0;
		for (int k = 0; k < Vec_Size; k++) {
			value += d_VecX[row * Vec_Size + k] + d_VecY[k * Vec_Size + k];
		}
		d_MatA[row * Vec_Size + col] = value;
	}
}
...
{
	dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE);
	dim3 dimGrid(ELEM_PER_VECTOR / dimBlock.x, ELEMENT_SIZE / dimBlock.y);
	Mat_Vec_Multiply_Kernel << <dimGrid, dimBlock >> > (p_VecX, p_VecY, p_VecY_G);
}
