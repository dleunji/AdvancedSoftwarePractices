			...
			for (int l = -w; l <= w; l++) {
				for (int k = -w; k <= w; k++) {
					if (r - k < 0 || c - l < 0 || r - k >= m_height || c - l >= m_width) {
						...
						Gx[r * m_width + c] += double(m_imageBits[(r - k) * m_width + (c - l)]) * Sx[w - k][w - l];
						Gy[r * m_width + c] += double(m_imageBits[(r - k) * m_width + (c - l)]) * Sy[w - k][w - l];
					}
				}
			}
			G[r * m_width + c] = sqrt(pow(Gx[r * m_width + c], 2) + pow(Gx[r * m_width + c], 2));
		}
	}
			...
			for (int l = -w; l <= w; l++)
				for (int k = -w; k <= w; k++) {
					if (r - k < 0 || c - l < 0 || r - k >= m_height || c - l >= m_width)
						...
						mean += imageBits[(r - k) * m_width + (c - l)] * kernel[w - k][w - l];
				}
				...
				for (int l = -w; l <= w; l++)
					for (int k = -w; k <= w; k++) {
						if (r - k < 0 || c - l < 0 || r - k >= m_height || c - l >= m_width)
							...
							mean += m_imageBits[(r - k) * m_width + (c - l)] * kernel[w - k][w - l];
					}
				m_imageBitsFiltered[r * m_width + c] = mean;

			}
			else {//c > 0
				for (int k = -w; k <= w; k++) { //row rotation
					if (r - k < 0 || r - k >= m_height || c - w - 1 < 0)
						mean -= 0; //continue 사용불가
					else
						mean -= m_imageBits[(r - k) * m_width + (c - w - 1)] * kernel[w - k][0];
					if (r - k < 0 || r - k >= m_height || c + w >= m_width)
						mean += 0;
					else
			...
			for (int l = -w; l <= w; l++) {
				for (int k = -w; k <= w; k++) {
					if (r - k < 0 || c - l < 0 || r - k >= m_height || c - l >= m_width)
						...
						medarray[i++] = m_imageBits[(r - k) * m_width + (c - l)];
				}
			}
	...
	for (r = 0; r < m_height; r++) {
		for (c = 0; c < m_width; c++) {
			if (c == 0) {
				...
				for (int i = 0; i < w0 - 1 ; i++) { // i : 0~3 //medarray 배열 정리
					for (int j = 0; j < w0; j++) {//j : 0~4
						medarray[w0 * (i + 1) + j] = medarray[w0 * i + j];
					}
				}
				i = 0;
				...
				for (int k = -w; k <= w; k++) { //row rotation
					if (r - k < 0 || r - k >= m_height || c - w - 1 < 0)
						...
						medarray[i] = m_imageBits[(r - k) * m_width + (c - w - 1)];

					if (r - k < 0 || r - k >= m_height || c + w >= m_width)
						medarray[i] = 0;
					else
						medarray[i] = m_imageBits[(r - k) * m_width + (c + w)];
					i += 1;
				}
				QuickMedian(medarray, 0, K - 1, K / 2);
				m_imageBitsFiltered[r * m_width + c] = medarray[K / 2];
