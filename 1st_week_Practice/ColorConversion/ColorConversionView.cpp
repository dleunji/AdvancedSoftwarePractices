...
void CColorConversionView::OnProblem1Sub1()
{
	...
	unsigned char g; //8bits
	unsigned int r, c; // row, column
	...
	for (r = 0; r < height; r++) {
		for (c = 0; c < width; c++) {
			g = m_dibFile.m_lpImage[r*width + c];
			mg_lpImage[r*width + c] = g/2;
		}
	}
...
void CColorConversionView::OnProblem2()
{
	...
	for (r = 0; r < height; r++) {
		for (c = 0; c < width; c++) {
			bl = m_dibFile.m_lpImage[r * width * bit + c*bit];
			bl *= 0.114;
			gr = m_dibFile.m_lpImage[r * width * bit + c*bit+1];
			gr *= 0.587;
			rd = m_dibFile.m_lpImage[r * width * bit + c*bit+2];
			rd *= 0.299;
			mg_lpImage[r * width + c] = bl + gr + rd;
		}
	}
...
void CColorConversionView::OnProblem3()
{
	...
	for (r = 0; r < height; r++) {
		for (c = 0; c < width; c++) {
			...
			rd = (red_mask & tempim16[r * width + c]) >> 10;
			rd *= 0.299;
			gr = (green_mask & tempim16[r * width + c]) >> 5;
			gr *= 0.587;
			bl = (blue_mask & tempim16[r * width + c]);
			bl *= 0.114;

			mg_lpImage[r * width + c] = (rd + gr + bl)<<3;
		}
	}
